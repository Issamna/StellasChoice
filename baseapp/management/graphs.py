...
data.loc[data['Health'] == 1, ['Health']] = 'Healthy'
data.loc[data['Health'] == 2, ['Health']] = 'Minor Injury'
data.loc[data['Health'] == 3, ['Health']] = 'Serious Injury'

# Distribution by health
data['Health'].value_counts().sort_index().plot(kind='bar')
plt.title('General Health Count')
plt.show()

# Distribution by Health & adoption speed
g = sns.countplot(x='Health', data=data, hue='AdoptionSpeed')
plt.xlabel('General Health')
plt.ylabel('Count')
plt.title('Adoption speed by General Health (Percentage)')
plt.show()
